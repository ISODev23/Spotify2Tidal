...
class Spotify:
    ...
    def tracks_from_playlist(self, playlist):
        ...
        """
        try:
            result = self.spotify_session.playlist_items(
                playlist_id=playlist["id"],
                fields="items(track(id,name,artists(name))),next",
                additional_types=("track",),
            )

            tracks = [item for item in result.get("items", []) if item.get("track")]

            while result.get("next"):
                result = self.spotify_session.next(result)
                tracks.extend(
                    item for item in result.get("items", []) if item.get("track")
                )
        except spotipy.client.SpotifyException:
            self._refresh_expired_token()
